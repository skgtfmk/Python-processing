...
#define table
table1 = TableBuilder.build('CRO_CACO2_PERM')
...
tcPappa2B = table1.getColumnByName('PAPPA2B_')
tcPappa2BQualifier = table1.getColumnByName('PAPPA2B_QUALIFIER')
tcPappb2A = table1.getColumnByName('PAPPB2A_')
...
tcRecovA2B = table1.getColumnByName('RECOV_A2B_')
...
logger.info('running script CRO_CACO2_PERM ...')
...
for i,r in enumerate(range(1,sheet1.getNumRows(),1)):
    ...
    row.addCell(tcCro, sheet1.getCellValue(r,5))
    row.addCell(tcPappa2B, sheet1.getCellValue(r,6))
    row.addCell(tcPappa2BQualifier, sheet1.getCellValue(r,7))
    row.addCell(tcPappb2A, sheet1.getCellValue(r,8))
    row.addCell(tcPappUnits, sheet1.getCellValue(r,9))
    row.addCell(tcPappb2AQualifier, sheet1.getCellValue(r,10))
    row.addCell(tcEffluxRatio, sheet1.getCellValue(r,11))
    row.addCell(tcEffluxRatioQualifier, sheet1.getCellValue(r,12))
    row.addCell(tcRecovA2B, sheet1.getCellValue(r,13))
    row.addCell(tcRecovA2BQualifier, sheet1.getCellValue(r,14))
    row.addCell(tcRecovB2A, sheet1.getCellValue(r,15))
    row.addCell(tcRecovB2AQualifier, sheet1.getCellValue(r,16))
    row.addCell(tcTransfection, sheet1.getCellValue(r,17))
    row.addCell(tcInhibitorPresent, sheet1.getCellValue(r,18))

logger.info('CRO_CACO2_PERM script finished')